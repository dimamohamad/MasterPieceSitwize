...
namespace MasterPiece.Controllers
{
    public class BookingController : Controller
    {
        ...
        public ActionResult BookSitter(int sitterId)
        {
            ...
            if (serviceDetail.DetailType == "Regular sitter")
            {
                var selectedDays = serviceDetail.Days?.Split(',')
                          .Where(day => day != "false")
                          .ToList() ?? new List<string>();
                int checkedDaysCount = selectedDays.Count;

                // Calculate cost based on the new formula
                totalCost = checkedDaysCount * 4 * durationInHours * hourlyRate;
            }
            else if (serviceDetail.DetailType == "Nanny sitter")
            {
                // Split the selected days by comma and trim spaces
                var selectedDays = serviceDetail.Days?.Split(',')
                              .Select(day => day.Trim()) // Trim spaces
                              .ToList() ?? new List<string>();

                // Debugging: Check the selected days
                System.Diagnostics.Debug.WriteLine("Selected Days: " + string.Join(", ", selectedDays));

                int checkedDaysCount = 0;

                // Check for Weekend (Weekend counts as 2 days)
                if (selectedDays.Any(day => day.IndexOf("Weekend", StringComparison.OrdinalIgnoreCase) >= 0))
                {
                    checkedDaysCount += 2; // Weekend is counted as 2 days
                }

                if (selectedDays.Any(day => day.IndexOf("Weekdays", StringComparison.OrdinalIgnoreCase) >= 0))
                {
                    checkedDaysCount += 5; // Weekdays are counted as 5 days
                }

                // Calculate cost based on the number of days
                totalCost = checkedDaysCount * 4 * durationInHours * hourlyRate;

            }
            else
            {
                // For non-regular services, use the regular duration calculation
